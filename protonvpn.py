...
PROTON_TLS_CRYPT = """-----BEGIN OpenVPN Static key V1-----
...
def generate_config(server_name, protocol, username, password):
    ip = get_server_ip(server_name)
    if not ip:
        return None, f"Server not found: {server_name}"

    proto = protocol.lower()

    if proto == "tcp":
        remotes = f"remote {ip} 7770\nremote {ip} 443\nremote {ip} 8443"
    else:
        remotes = f"remote {ip} 1194\nremote {ip} 80\nremote {ip} 4569\nremote {ip} 5060\nremote {ip} 51820"
